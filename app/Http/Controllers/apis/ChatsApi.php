...
class ChatsApi extends Controller
{
    function sendChatRequest(Request $request)
    {
        ...
        if ($request->type != 'home') {
            ...
            event(new SendFcmNotificationEvent([$second_user->fcm_token], 'تم ارسال طلب اليك', 'تم ارسال طلب اليك من' . $request->user()->name, ['chat_id' => $chat->id, 'sender_id' => $request->user()->id, 'is_accepted' => $chat->is_accepted, 'type' => $request->type,'chat_type'=>$request->type]));
            if ($chat_request == 1)
    ...
    function accept(Request $request)
    {
        ...
        if ($chat->type == 'friend_request')
            event(new SendFcmNotificationEvent([$chat->firstUser->fcm_token], 'تم الموافقه على الطلب الخاص بك', 'تم الموافقه على الطلب الخاص بك من طرف' . $chat->secondUser->name, ['chat_id' => $chat->id, 'sender_id' => $request->user()->id, 'is_accepted' => $chat->is_accepted, 'type' => 'chat_accepted','chat_type'=>'chat_accepted'], 'acceptOrReject'));
        return response()->json([
            'status' => true,
            'code' => 200,
            'data' => $chat->toArray()
...
//        if ($chat->type == 'friend_request')
            event(new SendFcmNotificationEvent([$chat->firstUser->fcm_token], 'تم رفض الطلب الخاص بك', ' تم رفض الطلب الخاص بك من طرف' . $chat->secondUser->name, ['chat_id' => $chat->id, 'sender_id' => $request->user()->id, 'is_accepted' => $chat->is_accepted, 'type' => 'chat_rejected','chat_type'=>'chat_rejected'], 'acceptOrReject'));
